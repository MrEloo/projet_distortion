...
class CategoryManager extends AbstractManager
{
    ...
    public function delete(int $id): void
    {
        ...
        $getChannels = $this->getAllChannelsFromCategory($id);

        foreach ($getChannels as $key => $getChannel) {
            $deleteQuery = $this->db->prepare('DELETE FROM posts WHERE id_salon = :id_salon');
            $parametersDeletePosts = [
                'id_salon' => $getChannel->getId(),
            ];
        ...
        $deleteQuery = $this->db->prepare('DELETE FROM channels WHERE id_cat = :id_cat');
        $parameters = [
            'id_cat' => $id
        ];
